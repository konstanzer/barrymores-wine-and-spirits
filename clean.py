...
def change_item_names(df):
	...
	df.replace('INN000','', regex=True, inplace=True)
...
def divvy_field(item, df, categories = ('LIQ', 'WIN', 'MER', 'WHI')):
		...
		for ix2, s in enumerate(substr):
			if 'B0' in s and len(s) > 30:
				...
				if suffix:
					...
					df.category[ix], df.subcategory[ix] = '', ''

				sku_raw = [i for i in s[:suffix].split('B') if len(i) > 10]
				df.item_no[ix] = sku_raw[1]
				#Adds the word attached to SKU to item name.
...
def clean_data(input_file, output_file):
	...
	'''
	df = pd.read_excel(path.join(dir_path, input_file), header=None)
	print('Data loaded.')
	df = change_item_names(df)
	item, df = select_columns(df)
	print('Calculating...')
	df = divvy_field(item, numeric_fields(df))
